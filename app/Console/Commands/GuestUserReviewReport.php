...
class GuestUserReviewReport extends Command
{
    ...
    public function getReport ($restaurant,$start_date,$end_date) {



        $query =  Question::where(["restaurant_id" => $restaurant->id,"is_default" => false]);
...
if(empty($restaurant->is_report_email_enabled)) {
    continue;
}
    ...
    $data =  $this->getReport($restaurant,$start_date_of_previous_week,$end_date_of_previous_week);


    $pdf = PDF::loadView('guest-user-review-report-pdf', compact("data","restaurant","start_date_of_previous_week","end_date_of_previous_week"));
    $pdfContents = $pdf->output();


$to=['drrifatalashwad0@gmail.com',$restaurant->EmailAddress,"asjadtariq@gmail.com"];
