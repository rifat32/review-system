...
class StripeController extends Controller
{
    ...
    public function redirectUserToStripe(Request $request)
    {
        ...
        $business = Restaurant::findOrFail($order->restaurant_id);
     ...
     *
     * @OA\Patch(
     *      path="/restaurant/UpdateResturantStripeDetails/{restaurentId}",
    ...
    public function UpdateResturantStripeDetails($restaurentId, Request $request)
    {
        ...
        if (!($checkRestaurant->pin == $request->header("pin"))) {
            return response()->json([
                "message" => "invalid pin"
            ], 401);
        }
        ...
        if ($checkRestaurant->OwnerID != $request->user()->id) {
     ...
     *
     * @OA\Get(
     *      path="/restaurant/getResturantStripeDetails/{id}",
     ...
     *     )
     */
    public function GetResturantStripeDetails($restaurantId)
    {
        $business =   Restaurant::with("owner")->where(["id" => $restaurantId])->first();


        if (!$business) {
            return response()->json([
                "message" => "you don't have a valid business" . $restaurantId
            ], 401);
        }
        ...
        $data["eat_in_payment_mode"] = $business->eat_in_payment_mode;
        $data["takeaway_payment_mode"] = $business->takeaway_payment_mode;
        $data["delivery_payment_mode"] = $business->delivery_payment_mode;
     ...
     *
     * @OA\Get(
     *      path="/client/restaurant/getResturantStripeDetails/{id}",
     *      operationId="GetResturantStripeDetails",
     *      tags={"restaurant"},
     *       security={
     *           {"bearerAuth": {}}
     ...
     *     )
     */
    public function GetResturantStripeDetailsClient($restaurantId)
    {
        $business =   Restaurant::with("owner")->where(["id" => $restaurantId])->first();
